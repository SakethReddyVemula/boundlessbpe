...
class BPEState:
    ...
    def __init__(self, prefix : str, tau: float, is_super : bool, unlocked: defaultdict[bytes,bool]):
        ...
        self.merge_pattern = re.compile(IMPROVED_MERGE_PATTERN)
    ...
    def get_stats_faster(self,
            ...
            # this should be unlocked too 
            self.unlocked[combined] = True
            ...
            while i < n-1:
                ...
                if tokens[i] == first and tokens[i+1] == second:
                    ...
                    if i-1 >= 0:
                        ...
                        if self.unlocked[prev] and self.can_merge(prev, combined):
                    ...
                    if k+2 < len(tokens):
                        ...
                        if self.unlocked[after] and self.can_merge(combined, after):
    ...
    def merge_and_update(self, max_pair : Tuple[bytes,bytes]) -> int:
        ...
        # only do this for is_super
        # TODO: work out logic for the first == second case
        if (first != second) and self.is_super:
            ...
            for tokens, cnt in zip(self.text_chunks, self.text_counts):
                ...
                if merge_cnt_before > 0:
                    ...
                    if len(tokens) == 1 and (before_len > 1):
                        ...
                        if not self.can_merge(first, second):
                            print("warning on can_merge", first, second, new_unlocked)
                    
                        # we'll return this too so we can affect superwords
                        self.unlocked[new_unlocked] = True
        else:

            # this is slower for is_super, but faster for regular merges
            for tokens, cnt in zip(self.text_chunks, self.text_counts):
    ...
    def get_replacement_parts(self, bad_token : bytes, blowup : bool) -> list[bytes]:
        ...
        if not blowup:
            ...
            for (left, right) in self.merges.values():
                if left + right == bad_token:
                    parts = [left,right]
                    break
            assert parts is not None, "couldn't find merge for " + str(bad_token)
            print("debug: found bad_token pair", bad_token, left, right)
    ...
    def delete_and_update(self, bad_token : bytes, blowup : bool) -> None:
        
        parts = self.get_replacement_parts(bad_token, blowup)
    ...
    def merge_and_delete(self, best_pair, i, vocablist, start_time, blowup, verbose):
            ...
            for (ios, tok, direction) in [(ios_a, left, "left"), (ios_b, right, "right")]:
                ...
                if (len(tok) > 1) and (ios >= self.tau):
                    ...
                    self.delete_and_update(tok, blowup)
...
class FasterHalfDirectRegexTokenizer(UniformTokenizer):

    def __init__(self, tau, pattern=None):
        """
        - pattern: optional string to override the default (GPT-4 split pattern)
        - special_tokens: str -> int dictionary of special tokens
          example: {'<|endoftext|>': 100257}
        ...
        self.words_state = BPEState('w', tau, is_super = False, unlocked=always_unlocked)

        always_locked : defaultdict[bytes, bool] = defaultdict(lambda: False)  # everything is unlocked by default

        # no deletion yet for superwords until we can figure out how to blow up correctly
        # use 1.1 here so we have no deletions
        self.superwords_state = BPEState('s', 1.1, is_super = True, unlocked=always_locked)
    ...
    def verify_indicies(self) -> None:
        ...
        indices = sorted(list(self.words_state.merges.keys()) + \
                         ...
                         list(self.superwords_state.merges.keys()) + \
                         list(self.superwords_state.deletions.keys()))
        ...
        for i, ind in enumerate(indices):
            ...
            if i != ind:
                ...
                for idx, pair in self.superwords_state.merges.items():
                    print(idx, frombytespair(pair))
                print()
                print("superword deletions:")
                for idx, tok in self.superwords_state.deletions.items():
                    print(idx, frombytes(tok))
                print()
    ...
    def train(self, 
        ...
        self.superwords_state.initial_counts()
        ...
        while len(self.vocablist) < vocab_size:
            ...
            if len(self.words_state.pair_counts) == 0 and len(self.superwords_state.pair_counts) == 0:
            ...
            best_pair_superwords, best_cnt_superwords = self.superwords_state.choose_best_pair(self.vocablist)

            bpw = "none"
            if best_pair_words is not None:
                bpw = frombytes(best_pair_words[0] + best_pair_words[1])
            bpsw = "none"
            if best_pair_superwords is not None:
                bpsw = frombytes(best_pair_superwords[0] + best_pair_superwords[1])

            print("best pair:", bpw, best_cnt_words, bpsw, best_cnt_superwords)

            total_max_value += (time.time() - start_max)
            
            # should break out of while loop if didn't find anything
            # because of min_cnt 
            if best_pair_words is None and best_pair_superwords is None:
                print("best is None for both words and superwords")
                break

            ########### 

            # do we want to do the word or superwords?
            # at least one of these must be not None
            if best_cnt_words > best_cnt_superwords:
                #this side effects self.vocablist as necessary
                merge_time, delete_time, new_unlocked = self.words_state.merge_and_delete(best_pair_words, i, self.vocablist, start_time, blowup, verbose)

                start_unlocked = time.time()
                # doesn't always make a whole word
                if new_unlocked is not None:
                    self.superwords_state.unlock_and_update(new_unlocked)

                unlocked_time = time.time() - start_unlocked

                total_unlocked += unlocked_time

            else:
                #this side effects self.vocablist as necessary                
                # don't care about new_unlocked here
                merge_time, delete_time, _ = self.superwords_state.merge_and_delete(best_pair_superwords, i, self.vocablist, start_time, blowup, verbose)
            ...
            print("timing:", time.time() - start_time, merge_time, delete_time, unlocked_time)
            ...
            if i % recalc == 0 and i > 0:
                ...
                self.superwords_state.verify_state()
            ...
            if len(self.vocablist) % 8192 == 0:  #  100 == 0:
                ...
                print(":super_single_counts:", len(self.superwords_state.single_counts))
                print(":super_pair_counts:", len(self.superwords_state.pair_counts))
                print(":super_single_byte_cnt:", self.superwords_state.get_single_byte_cnt())
                print(":super_whole_words:", self.superwords_state.whole_words)
                print(":super_merges:", len(self.superwords_state.merges))
                print(":super_deletions:", len(self.superwords_state.deletions))
                ...
                print(":missing:", overall_time - total_pretok - total_ic - total_max_value - total_merge - total_delete - total_verify - total_unlocked)
    ...
    def save(self, file_prefix):
        ...
        with open(model_file, 'wt') as f:
            ...
            _write_sorted_dict_intkey(self.superwords_state.merges, f, ispair=True, isstr=False)  # these have int as the key, since tokens might not be unique          
            _write_sorted_dict_intkey(self.superwords_state.deletions, f, ispair=False, isstr=False)
    ...
    def load(self, model_file):
        ...
        with open(model_file, 'rt', encoding="utf-8") as f:
            ...
            self.superwords_state.merges  = _read_sorted_dict_intkey(f, ispair=True, isstr=False)
            self.superwords_state.deletions  = _read_sorted_dict_intkey(f, ispair=False, isstr=False)
    ...
    #         # TODO: add super merges here
    #         pair = self.min_merge_or_deletion(tokens)
    #         if pair is None:
